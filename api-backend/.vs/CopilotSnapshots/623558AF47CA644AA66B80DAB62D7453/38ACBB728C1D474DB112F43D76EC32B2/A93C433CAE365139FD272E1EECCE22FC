...
#warning To protect potentially sensitive information in your connection string, you should move it out of source code. You can avoid scaffolding the connection string by using the Name= syntax to read it from configuration - see https://go.microsoft.com/fwlink/?linkid=2131148. For more guidance on storing connection strings, see https://go.microsoft.com/fwlink/?LinkId=723263.
    ...
    {
        // Apply all configurations from the Configurations folder using IEntityTypeConfiguration
    ...
    {
        // Only configure entities that don't have their own IEntityTypeConfiguration classes
        ...
        // Continue with other entities that don't have configurations...
    ...
    {
        // Quiz entities that don't have their own configuration classes yet
        modelBuilder.Entity<QuizAnswer>(entity =>
        {
            entity.HasKey(e => new { e.AttemptId, e.QuestionId, e.OptionId }).HasName("PK__QuizAnsw__A754A9BDB12CC998");
            entity.HasOne(d => d.Attempt).WithMany(p => p.QuizAnswers).HasForeignKey(d => d.AttemptId).HasConstraintName("FK__QuizAnswe__Attem__30C33EC3");
            entity.HasOne(d => d.Option).WithMany(p => p.QuizAnswers).HasForeignKey(d => d.OptionId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK__QuizAnswe__Optio__32AB8735");
            entity.HasOne(d => d.Question).WithMany(p => p.QuizAnswers).HasForeignKey(d => d.QuestionId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK__QuizAnswe__Quest__31B762FC");
        });

        modelBuilder.Entity<QuizAttempt>(entity =>
        {
            entity.HasKey(e => e.AttemptId).HasName("PK__QuizAtte__891A68E6788FD249");
            entity.Property(e => e.GradedAt).HasPrecision(0);
            entity.Property(e => e.ScoreRaw).HasColumnType("decimal(10, 2)");
            entity.Property(e => e.ScoreScaled10).HasColumnType("decimal(4, 2)");
            entity.Property(e => e.StartedAt).HasPrecision(0).HasDefaultValueSql("(sysutcdatetime())");
            entity.Property(e => e.Status).HasMaxLength(20).IsUnicode(false).HasDefaultValue("in_progress");
            entity.Property(e => e.SubmittedAt).HasPrecision(0);
            entity.HasOne(d => d.GradedByNavigation).WithMany(p => p.QuizAttemptGradedByNavigations).HasForeignKey(d => d.GradedBy).HasConstraintName("FK__QuizAttem__Grade__2DE6D218");
            entity.HasOne(d => d.Quiz).WithMany(p => p.QuizAttempts).HasForeignKey(d => d.QuizId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK__QuizAttem__QuizI__2BFE89A6");
            entity.HasOne(d => d.Student).WithMany(p => p.QuizAttemptStudents).HasForeignKey(d => d.StudentId).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK__QuizAttem__Stude__2CF2ADDF");
        });

        // Add more quiz entities as needed...
    }
